...
if AUTH_AVAILABLE:
    ...
    st.warning("⚠️ Authentication system tidak tersedia. Mode development.")
...
if SERVICES_AVAILABLE:
    ...
    st.error("❌ System services tidak tersedia. Check deployment logs.")
...
def display_logo():
    ...
    if not SERVICES_AVAILABLE:
        ...
        <div style="text-align: center; margin-bottom: 2rem;">
            <h3 style="color: #667eea; margin: 10px 0 5px 0;">VPants</h3>
            <p style="color: #764ba2; font-weight: bold; margin: 0;">SMART WOMEN</p>
            <p style="font-size: 0.8rem; color: #666; margin: 5px 0;">Cp: 085157149669</p>
        </div>
...
if AUTH_AVAILABLE:
    ...
    <div style="background: #f8f9fa; padding: 1rem; border-radius: 10px; margin-bottom: 1rem;">
        <p style="margin: 0; font-size: 0.9rem;">
            <strong>User:</strong> {get_current_user().title()}<br>
...
# Show warning if services not available
if not SERVICES_AVAILABLE:
    st.error("""
    ❌ System services tidak tersedia. 
    ...
    Silakan check deployment logs atau hubungi administrator.
    """)
    st.stop()
...
# Header
st.markdown(f'<div class="main-header">👙 VPants - Sistem Pembukuan & Stok</div>', unsafe_allow_html=True)

# [REST OF YOUR ORIGINAL APP CODE...]
# Copy semua kode dari app.py setelah header
from services.sales_service import SalesService
from models.transaction import Transaction
from models.stock import StockItem
from utils.helpers import format_currency, safe_float
